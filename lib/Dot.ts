...
const dot_w = 120;			// dot width
const dot_h = 120;			// dot height
const graph_w = 325;		// graph width
const graph_h = 120;		// graph height
const inv_graph_h = 1.0 / graph_h;
const shadowOffset = 10;	// graph drop shadow offset
const shadowBlur = 2.75;	// dot drop shadow blur
...
var shadowSvg = `<svg xmlns='http://www.w3.org/2000/svg' preserveAspectRatio='xMidYMid meet' width='${dot_w}' height='${dot_h}' style='opacity:0.7'>
    <defs>
        <filter id='fh'>
...
var dotSvg = `<svg xmlns='http://www.w3.org/2000/svg' preserveAspectRatio='xMidYMid meet' width='${dot_w}' height='${dot_h}'>
    <defs>
        <filter id='f1'>
...
var graphSvg = `<svg xmlns='http://www.w3.org/2000/svg' preserveAspectRatio='xMidYMid meet' width='${graph_w}' height='${graph_h}'>
    <defs>
...
export async function renderDot(): Promise<[number, Canvas]> {
    ...
    const dotData = JSON.parse(xml2json(await request({
        method: 'GET',
        timeout: 10000,
    ...
    // the server sends back a full minute of data, we only need the current second
    let serverTime = dotData[0].elements[0].text;
    let currentDotValue = Number(dotData[1].elements.find((item: any) => {
        return item.attributes.t === serverTime
    }).elements[0].text);

    // gcp nerd math for blending dot colors together
    for (let i = 0; i < dotColors.length - 1; i++) {
        var opacity = (currentDotValue - dotColors[i].tail) / (dotColors[i + 1].tail - dotColors[i].tail);
...
export async function renderDotGraph(timespan: number): Promise<Canvas> {
	// create canvases for graph and graph shadow, to be composited onto output canvas
    var canvasShadow = createCanvas(graph_w, graph_h+shadowOffset*2);
    var canvas = createCanvas(graph_w, graph_h);
    var outCanvas = createCanvas(graph_w, graph_h+shadowOffset*2);
    var contextShadow = canvasShadow.getContext('2d');
    var context = canvas.getContext('2d');
    var outContext = outCanvas.getContext("2d");

    const graphData = JSON.parse(xml2json(await request({
        method: 'GET',
        timeout: 10000,
        url: `http://global-mind.org/gcpdot/gcpgraph.php?pixels=${graph_w}&seconds=${timespan}`,
        json: false,
    }))).elements[0].elements;
    ...
    let imgBuffer = context.getImageData(0, 0, graph_w, graph_h);
    for (let y = 0; y < graph_h; y++) {
        for (let x = 0; x < graph_w; x++) {
            imgBuffer.data[(y * graph_w + x) * 4 + 3] = 0;
        }
    }

    let imgShadowBuffer = contextShadow.createImageData(bgImage.width, bgImage.height + shadowOffset * 2);

    for (let i = 0; i < graphData.length; i++) {
        let top = parseFloat(graphData[i].attributes.t);
        let bottom = parseFloat(graphData[i].attributes.b);
        let q1 = parseFloat(graphData[i].attributes.q1);
        let q3 = parseFloat(graphData[i].attributes.q3);

        if ((bottom - top) < inv_graph_h) {
            if (top > 0.5) {
                top -= inv_graph_h;
            }
        }
        for (let y = Math.floor(top * graph_h); y < bottom * graph_h; y++) {
            let ys = y / graph_h;
            let a = 0;
            if (ys > q1 && ys <= q3 || (bottom - top) < inv_graph_h * 1.5) {
                a = 1;
...
function stackBlurCanvasAlpha(pixels: Uint8ClampedArray, width: number, height: number, radius: number) {
