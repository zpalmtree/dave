...
def main():
    ...
    hashes = json.loads((archive / "video-recovery-hashes.json").read_text())
    for name in ("gpuq_settings.py", "video_recovery.py", "test_video_recovery.py"):
        target = args.directory / name
        expected = (archive / name).read_bytes()
    ...
    print("Applied and verified the desktop recovery patch.")
