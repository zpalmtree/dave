#!/usr/bin/env python3
"""Install the audited recovery telemetry update on the desktop worker."""
import argparse
import hashlib
import json
...
def main():
    ...
    hashes = json.loads((archive / 'video-recovery-telemetry-hashes.json').read_text())
    ...
    if all(current[name] == expected['after_sha256'] for name, expected in hashes.items()):
        print('Desktop recovery telemetry matches the audited revision.')
        return
    if any(current[name] != expected['before_sha256'] for name, expected in hashes.items()):
        raise SystemExit('Unexpected desktop edits; inspect before installing recovery telemetry.')
    command = ['git', 'apply', str(archive / 'video-recovery-telemetry.patch')]
    subprocess.run([*command[:2], '--check', command[2]], cwd=args.directory, check=True)
    if args.check:
        print('Recovery telemetry patch applies to the audited desktop files.')
        return
    ...
    print('Applied and verified desktop recovery telemetry.')
