...
def main():
    ...
    hashes = json.loads((archive / "video-fallback-delivery-hashes.json").read_text())
    states = []
    ...
    if set(states) == {"after"}:
        print("Desktop fallback delivery files match the tested revision.")
        return
    if set(states) != {"before"}:
        raise SystemExit("Mixed desktop revisions; inspect before applying the patch.")
    patch = str(archive / "video-fallback-delivery.patch")
    ...
    print("Applied and verified the desktop fallback delivery patch.")
