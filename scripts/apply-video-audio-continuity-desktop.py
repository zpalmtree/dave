...
def main():
    ...
    hashes = json.loads((archive / "video-audio-continuity-hashes.json").read_text())
    states = []
    ...
    if set(states) == {"after"}:
        print("Desktop audio continuity files match the tested revision.")
        return
    if set(states) != {"before"}:
        raise SystemExit("Mixed desktop revisions; inspect before applying the patch.")
    patch = str(archive / "video-audio-continuity.patch")
    ...
    print("Applied and verified the desktop audio continuity patch.")
