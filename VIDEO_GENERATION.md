...
- `$minimax --replace "subject to replace" <direction>` and `$oalgo --replace "subject to replace" <direction>` use an attached or replied-to video clip as the source timeline. Attach one replacement image for either command; `$oalgo` uses its built-in portrait if no image is attached. A natural prompt beginning `replace X with Y` also selects replacement when a clip is present. The clip must be 5–15 seconds and at most 100 MiB. SAM3 tracks the named subject, MiniMax H3 Ref2VA and Fun ControlNet repaint its mask using the image, and the final composition retains the source background outside the mask and the original soundtrack. If the target cannot be isolated, the job fails instead of silently returning the unedited clip. Video replacement uses a separate workflow and does not accept a second song or `--image-provider`. See [video replacement](docs/video-source-edit.md).
