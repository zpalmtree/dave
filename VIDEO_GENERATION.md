...
- `$oalgo <prompt>` (also `$minimutt`) runs the same MiniMax H3 pipeline with the built-in OALGO starting image and its character-specific dialogue guidance. `$meximutt <prompt>` uses that pipeline while requiring every spoken line to have an exaggerated cholo/ese-style Mexican-American Spanglish accent and hard-edged street cadence; generated dialogue aggressively code-switches and uses expressions such as “ese,” “güey,” “cabrón,” “pinche,” and “no mames.” If another image is attached or inherited from a reply, the bot first tries to AI-composite it with OALGO. Every composite must pass visual review against the original portrait for facial anatomy, body proportions, and hair, while retaining the attached subjects and scene meaning. Correctable gaze and pose differences are left to the subsequent screenplay; they do not invalidate an otherwise faithful source composite. Composites use GPT Image edits with both original inputs; a rejected candidate receives one corrective edit using those same originals. Composition has a five-minute deadline; failed or unavailable identity review falls back to the built-in image. An unreviewed composite cannot become the video’s identity reference.
