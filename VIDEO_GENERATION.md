...
- `$minimax --replace "subject to replace" <direction>` and `$oalgo --replace "subject to replace" <direction>` use an attached or replied-to video clip as the source timeline. Attach one replacement image for either command; `$oalgo` uses its built-in portrait if no image is attached. A natural prompt beginning `replace X with Y` also selects replacement when a clip is present. The clip must be 5–15 seconds and at most 100 MiB. SAM3 tracks the named subject, MiniMax H3 Ref2VA uses the replacement image, and Fun ControlNet repaints an expanded region around the tracked subject using the source clip. The final composition retains the source background outside that region and the original soundtrack. If the target cannot be isolated, the job fails instead of silently returning the unedited clip. Large differences in subject shape or a portrait reference in a full-body shot can crop the replacement. Video replacement uses a separate workflow and does not accept a second song or `--image-provider`. See [video replacement](docs/video-source-edit.md).
