...
- `$minimax` and `$oalgo` also accept an attached or replied-to song for original-audio lip-sync. Attach one MP3, WAV, FLAC, OGG/Opus, M4A or AAC file (1–120 seconds, up to 25 MiB), optionally with an image. The song determines the duration and remains the soundtrack. See [original-song lip-sync](docs/video-source-audio.md).
- For prompts other than `replace X with Y`, a video clip attached to the command, or to the message it replies to, supplies a frame as the starting image. The broker probes the clip with ffmpeg over HTTPS range requests and stores the middle frame as PNG. If that frame is nearly black, it tries the 25% and then the 75% points. ffmpeg may only read HTTPS and plain video containers, so an uploaded playlist cannot make the broker fetch other URLs. If ffmpeg is missing or fails, the broker falls back to Discord's media-proxy first frame. The planner is told the image came from a clip. An attached image takes precedence over a clip, and the command message's own attachment takes precedence over the replied message's.
- `$minimax replace X with Y` and `$oalgo replace X with Y` use an attached or replied-to video clip as the source timeline. Attach one replacement image for either command; `$oalgo` uses its built-in portrait if no image is attached. Quote X when it contains the word “with,” for example `replace "the woman with a red coat" with the attached image`. The clip must be 0.5–120 seconds and at most 100 MiB. Clips under five seconds are padded for H3 and trimmed back to the source length; clips over 15 seconds are split into frame-aligned renders and joined with the original audio. A seam may be visible where independently rendered segments meet. SAM3 tracks the named subject, MiniMax H3 Ref2VA uses the replacement image, and Fun ControlNet repaints an expanded region around the tracked subject using the source clip. The final composition retains the source background outside that region and the original soundtrack. If the target cannot be isolated, the job fails instead of silently returning the unedited clip. Large differences in subject shape or a portrait reference in a full-body shot can crop the replacement. Video replacement uses a separate workflow and does not accept a second song or `--image-provider`. See [video replacement](docs/video-source-edit.md).
