...
- `$oalgo <prompt>` (also `$meximutt` and `$minimutt`) runs the same MiniMax H3 pipeline with the built-in OALGO starting image and its character-specific dialogue guidance. If another image is attached or inherited from a reply, the bot first tries to AI-composite it with OALGO and falls back to the built-in image if composition fails.
