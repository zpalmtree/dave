...
test('OALGO composite instructions preserve both inputs as one scene', () => {
    ...
    assert.match(plan.keyframe.prompt, /Meximutt base image/);
    ...
    assert.equal(plan.keyframe.motion_contract.camera_relation.includes('Meximutt'), true);
});
...
test('short and long replacement clips wait for the updated edit worker', async () => {
    for (const duration of [2.2, 31]) {
        ...
        try {
            ...
            assert.equal(accepted.status, 201);
            const job = (await accepted.json()).job;
            assert.equal(job.requested_duration_seconds, duration);
            const connect = async version => {
                const socket = new WebSocket(`ws://127.0.0.1:${broker.listeningPort()}/v1/worker`, {
                    headers: { authorization: 'Bearer worker-secret' },
                });
            ...
            try {
                ...
                assert.equal(lease.job.id, job.id);
                assert.equal(lease.job.source_video_seconds, duration);
            } finally {
